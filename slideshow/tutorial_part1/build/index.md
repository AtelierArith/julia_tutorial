...
```
  ...
  * `julia script.jl` するごとにパッケージのロード，関数のコンパイルが行われるそのためのコストは無視できない
...
# まとめ
  ...
  * 試す・触る側 -> 使う側 -> 使いこなす側 -> 作る -> 作ったものを使ってもらう (割と大変)
  * Julia言語を多くの人に試してもらい，作る側の立場に立てる方が増えれば幸いです
